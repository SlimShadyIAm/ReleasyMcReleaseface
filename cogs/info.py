...
class Utilities(commands.Cog):    
    ...
    async def info(self, ctx):
        ...
        if len(res) == 1:
            ...
            embed = Embed(title=f"Info for {ctx.guild}", color=Color(value=0x37b83b), description=desc)
            ...
            for i, device in enumerate(devices, start=1):
                ...
                if role is not None and role.is_default():
                    embed.add_field(name=device, value=f"@everyone will be pinged", inline=True)
                elif guild_info[i] == 0:
                    embed.add_field(name=device, value="No role will be pinged", inline=True)
                elif guild_info[i] == -1:
                    embed.add_field(name=device, value="Not subscribed", inline=True)
                else:
                    if role is not None:
                        embed.add_field(name=device, value=f"{role.mention} will be pinged", inline=True)
                    else:
                        embed.add_field(name=device, value="An invalid role is set, please fix this! Please use `.subscribe` (see `.help subscribe` for details)", inline=False)
